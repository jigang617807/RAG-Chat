...
项目启动时，JPA 的 ddl-auto: update 配置会自动创建所需的数据表 (user, document, document_chunk, conversation, chat_message)。
3. 项目配置
...
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/simple_demo?useSSL=false&serverTimezone=UTC
...
ZHIPU_API_KEY: "你的key.xxxxxxxx"

# 方式二：设置系统环境变量 (推荐)
# Windows: set ZHIPU_API_KEY=你的key
...
⚠️ 注意事项
向量检索性能: 本项目为了演示简便，将 Embedding 向量以 JSON 格式存在 MySQL 中，并在内存中进行余弦相似度计算。这仅适用于小型演示项目。如果文档量巨大，请接入 Milvus、PgVector 或 Elasticsearch。
API 费用: 使用 Embedding 和 Chat 模型会消耗 Token，请关注智谱 AI 控制台的额度使用情况。
文件路径: 上传的文件默认保存在项目根目录下的 uploads/ 文件夹中。
📸 效果预览
上传文档: 支持拖拽上传 PDF，后台自动进行文本清洗和切片。
对话界面: 仿 Apple 风格的极简 UI，支持流式输出，体验流畅。
文档列表: 查看已上传文档，支持一键删除（同时清理所有相关数据）。
Enjoy Coding! 🎉
